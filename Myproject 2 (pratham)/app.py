...
# Get route, distance, and duration from OSRM API
def get_osm_route(start, end):
    ...
    if response.status_code == 200:
        ...
        if data["routes"]:
            ...
            duration = route_info["duration"] / 60    # in minutes
...
def index():
    if request.method == "POST":
        ...
        if start_location and end_location:
            ...
            if route:
                route = [(lat, lon) for lon, lat in route]  # Flip coordinates for folium
                folium.PolyLine(route, color="blue", weight=5, opacity=0.7).add_to(m)

                mid_point = [(start_location[0] + end_location[0]) / 2, (start_location[1] + end_location[1]) / 2]
                folium.Marker(
                    location=mid_point,
            ...
            # Render result page
            return render_template("result.html", distance=distance, duration=duration)

        return render_template("index.html", error="Could not find one or both locations.")
